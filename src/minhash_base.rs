/*
Shared utilities for minhash deduplication (used by both memory/disk)
...
======================================================================*/
/*
FileMap is an object to map local files to indices to save on storage space.
Essentially it just maps {local_file_path -> index} (and vice versa).

This needs to be globally stored in a way that it knows about ALL global files we want to run minhash on.
- In a single-node setting, or other scenarios in which the whole data pool can live on a local file system,
  nothing needs to be done, and this can be generated by rust.
- In the multi-node setting, where data lives on S3, this needs to be generated with the python tool (python/file_map_builder.py)

The rust struct is just a wrapper for a json mapping {path (local) -> index}


NOTES:
    In a single node setting, we only require that all .jsonl(.zst|.zstd|.gz) files within `local_input` are what we want
    to run deduplication on.

    In the multinode setting, if all the data can fit locally on some LFS, then it had better live in ONE directory
    (including all subdirectories). If using remote file storage, like in S3, then you want to arrange your remote/local
    ...
    Then you want your local files to look like:
    /lfs
    └── path/
        └── data/
            ├── alpha.jsonl
    ...
    where `local_input` should be `/lfs/path/data`

PARALLELISM STRATEGY: This should be global:
    - if data lives on s3 or some other remote storage system, use the python tool to make filemap.json.gz
    - if the entire dataset is local, this rust function works just fine
*/
...
======================================================================*/
/*
This step does all the hashing of the documents. It stores the signatures in
documents with a .sig.bin file according to the schema:
...
Where the:
- band_id (XXX) ranges from 0..num_bands (specified in the minhash config)
- sigchunk_id (YYY) ranges from 0..num_sigchunks (specified in the minhash config)
- pathchunk (ZZZ) ranges from 0..num_path_chunks (in the args of this function [will always be 0 in 'memory'/single-node setting])
...
Ultimately there should be |num_bands| x |num_sigchunks| of these files per path_chunk
...
pub fn clean_files(
    ...
    path_chunk_files
        ...
        .for_each(|(path, path_id)| {
            ...
            let (lines_seen, lines_removed) = clean_path2(
